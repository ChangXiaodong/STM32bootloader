...
if __name__ == "__main__":
    ...
    if(len(opts)>0):
        for option,value in opts:
            if option == '-p':
                serialPortinput = str(value)
                bljobs = BootLoaderJobs(serialPortinput)
                bljobs.initialChip()
                bljobs.releasePort()
                exit(0)
            elif option == '-l':
                ...
                for portnum in range(len(portlist.portList)):
                    print portlist.portList[portnum][1]
                exit(0)
            elif option == '-h':
                print ""
    ...
    portlist.enumerate_serial_ports()
    for portnum in range(len(portlist.portList)):
        serialPort.append(portlist.portList[portnum][1])

    for port in range(len(serialPort)):
        bljobs = BootLoaderJobs(serialPort[port])
        bljobs.initialChip()
        bljobs.releasePort()
